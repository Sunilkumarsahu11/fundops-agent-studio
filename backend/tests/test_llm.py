    ...
    plan = LLMPlan(
        ...
        tools=[
            LLMToolSelection(
                ...
                reason="Compare administrator and manager records.",
                confidence=0.96,
            ),
            LLMToolSelection(
                tool="build_exception_report",
                reason="Produce an exception report.",
                confidence=0.94,
            ),
        ],
        steps=[
            ...
            LLMWorkflowStep(
                tool="build_exception_report",
                input={"reconciliation_result": "$step_1"},
            ),
        ],
    )
    blueprint = planner._to_blueprint(
        FactoryRequest(request="reconcile administrator and manager NAV"),
        plan,
    )
    assert blueprint.metadata["planner"] == "langchain-openai-responses"
    assert [step.tool for step in blueprint.steps] == [
        "reconcile_records",
