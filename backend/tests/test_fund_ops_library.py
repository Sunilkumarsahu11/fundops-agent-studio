...
def test_library_catalog_contains_all_agents():
    ...
    assert ids == {
        "fund-reconciliation", "excel-quality", "capital-call-review", "nav-review",
        "valuation-review", "normalization", "portfolio-exposure", "investor-reporting",
        "exception-investigation", "fund-data-qa",
    }


def test_reconciliation_agent_uses_deterministic_engine_and_evidence():
    library = FundOperationsLibrary()
    result = library.execute("fund-reconciliation", AgentInput(parameters={
...
def test_excel_quality_agent_executes_shared_tool():
    ...
    result = FundOperationsLibrary().execute("excel-quality", AgentInput(parameters={
        "file_name": "quality.xlsx", "content_base64": __import__("base64").b64encode(buffer.getvalue()).decode(),
    }))
...
def test_exception_investigation_agent_executes_shared_tool():
    result = FundOperationsLibrary().execute("exception-investigation", AgentInput(parameters={
        "exceptions": [
            {"record_id": "2", "code": "MISSING_NAV", "severity": "high"},
            {"record_id": "1", "code": "DATE_VARIANCE", "severity": "critical"},
        ]
    }))
    assert result.status == "exceptions"
    assert [x["severity"] for x in result.result["exceptions"]] == ["critical", "high"]


def test_data_qa_does_not_fabricate_records():
    result = FundOperationsLibrary().execute("fund-data-qa", AgentInput(
